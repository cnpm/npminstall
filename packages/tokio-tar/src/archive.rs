...
impl<R: Read + Unpin> ArchiveBuilder<R> {
    ...
    pub fn build(self) -> Archive<R> {
        ...
        Archive {
            ...
            buf: Arc::new(SyncMutex::new(BytesMut::with_capacity(512))),
        }
    }
}
...
fn poll_next_raw<R: Read + Unpin>(
    ...
    let buf = archive.buf.clone();
    let mut buf = match buf.try_lock() {
        Ok(buf) => buf,
        Err(_) => return Poll::Pending,
    };
    let header_slice_start = buf.len();
    let mut header_pos = *next;
    let header_size = 512;

    loop {
        // Seek to the start of the next header in the archive
        if *next > archive.inner.pos.load(Ordering::SeqCst) {
        ...
        let header_bytes = unsafe {
            let all_slice = std::slice::from_raw_parts_mut(buf.as_mut_ptr(), header_size);
            &mut all_slice[header_slice_start..]
        };

        // EOF is an indicator that we are at the end of the archive.
        match futures_core::ready!(poll_try_read_all(&mut archive, cx, header_bytes)) {
            Ok(size) => {
                unsafe {
                    buf.set_len(header_slice_start + size);
                }

                if (header_slice_start + size) == header_size {
                    // read header done
                } else if header_slice_start + size == 0 {
    ...
    header.as_mut_bytes().copy_from_slice(&buf[0..]);
    ...
    // let mut buf = archive.buf.try_lock().unwrap();
    // buf.drain(0..).collect::<Vec<u8>>();
    buf.truncate(0);
    ...
    let mut remaining = entry.size;
    {
        ...
        if gnu.is_extended() {
            ...
            while ext.is_extended() {
                ...
                {
                    Ok(size) if size == ext.as_bytes().len() => {}
                    Ok(_) => return Poll::Ready(Err(other("failed to read extension"))),
