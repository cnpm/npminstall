...
const { NpmFs } = require('./rapid-mode/npm_fs/constants');
const rapidInstall = require('../lib/rapid-mode/install').install;
...
module.exports = async (options, context = new Context()) => {
  ...
  try {
    if (options.fs === NpmFs.rapid) {
      await rapidInstall(options);
    } else {
