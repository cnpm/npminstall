...
exports.copyInstall = function* (src, options) {
  ...
  if (!realPkg.name || !realPkg.version) {
    throw new Error('package.json must contains name and version');
  }

  const targetdir = path.join(options.storeDir, realPkg.name, realPkg.version);
  ...
  if (options.cache[key]) {
    console.log('exist cache', options.cache[key]);
