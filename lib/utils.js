 ...
 */
exports.pruneJSON = async (filepath, key) => {
  const pkg = await this.readJSON(filepath);
  const dep = pkg.dependencies;
  const devDep = pkg.devDependencies;
  if (this.hasOwnProp(dep, key)) {
    Reflect.deleteProperty(dep, key);
  }
  if (this.hasOwnProp(devDep, key)) {
    Reflect.deleteProperty(devDep, key);
  }
  this.addMetaToJSONFile(filepath, {
    dependencies: dep,
