...
function* _install(parentDir, pkg, ancestors, options) {
  ...
  if (options.flatten || forceFlatten(pkg)) {
    const res = yield matchAncestorDependencies(p, ancestors, options);
    if (res) {
  ...
  try {
    ...
    if (Object.keys(peerDependencies).length) {
      ...
      for (const name in peerDependencies) {
        ...
        // check in reverse
        const res = yield matchAncestorDependencies(childPkg, ancestors.slice().reverse(), options);
        if (res) {
          pkgs.push({ name, version: res.ancestorSpec });
          options.paddingMessages.push([
            'info',
            '%s %s peerDependency %s will using %s\'s dependency %s(%s)',
            chalk.cyan('peerDependencies'),
            chalk.gray(displayName),
            chalk.yellow(`${name}@${version}`),
            chalk.yellow(res.ancestor),
            chalk.yellow(`${name}@${res.ancestorSpec}`),
            chalk.yellow(res.ancestorResolved),
          ]);
        } else {
