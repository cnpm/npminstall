    ...
    if (Object.keys(peerDependencies).length) {
      ...
      for (const name in peerDependencies) {
        ...
        try {
          ...
          const childPkg = npa(pkg, options.root);
          // check in reverse
          ...
          // if a peer dependency install failed, we don't need to throw a error
          // just skip current installation. e.g.: react-hovertable@0.3.0
          console.warn(`[npminstall] install dependency ${pkg} error: `, error.message);
          continue;
        }
      }
