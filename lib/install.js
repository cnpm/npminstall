...
function* _install(parentDir, pkg, options) {
  ...
  if (Array.isArray(realPkg.os) && realPkg.os.length > 0) {
    let match = false;
    const platform = process.platform;
    for (const name of realPkg.os) {
      if (name === platform) {
        match = true;
        break;
      }
      if (name[0] === '!' && name.substring(1) === platform) {
        match = false;
        break;
      }
    }
    if (!match) {
      ...
      const err = new Error('Package require os(' + realPkg.os.join(', ') +
        ') not compatible with your platform(' + platform + ')');
