...
function* _install(parentDir, pkg, ancestors, options) {
  ...
  if (options.flatten || utils.endsWithX(pkg.version)) {
  ...
  if (semver.satisfies(resolveAncestorPkg.version, childPkg.spec)) {
    ...
    if (resolveChildPkg.version !== resolveAncestorPkg.version) {
      options.paddingMessages.push([
        ...
        chalk.yellow(`${childPkg.name}@${childPkg.rawSpec}`),
        chalk.yellow(resolveChildPkg.version),
        chalk.gray(ancestorPkg.parent),
        chalk.yellow(`${childPkg.name}@${ancestorPkg.rawSpec}`),
        chalk.yellow(resolveAncestorPkg.version),
      ]);
    }
    debug('%s %s delcares %s(resolved as %s) but using ancestor(%s)\'s dependency %s(resolved as %s)',
      'anti semver',
