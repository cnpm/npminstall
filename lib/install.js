...
async function _install(parentDir, pkg, ancestors, options) {
  ...
  let p = npa(pkg.name ? `${pkg.name}@${pkg.version}` : pkg.version, options.root)(options.nested);
  ...
  if (options.flatten || forceFlatten(pkg)) {
    ...
    if (res) {
      ...
      // use ancestor's spec
      p = npa(`${res.name}@${res.ancestorSpec}`, options.root)(options.nested);
  ...
  try {
    ...
    if (Object.keys(peerDependencies).length) {
      ...
      for (const name in peerDependencies) {
        ...
        options.nested.update([ raw ], p.raw);
        ...
        const childPkg = npa(raw, options.root)(options.nested);
        // check in reverse
    ...
    if (pkgs.length > 0) {
      ...
      options.nested.update(needPkgs.map(pkg => `${pkg.name}@${pkg.version}`), `${realPkg.name}@${realPkg.version}`);
...
async function matchAncestorDependencies(childPkg, ancestors, options) {
  ...
  for (const ancestor of ancestors) {
    ...
    const ancestorPkg = npa(`${childPkg.name}@${ancestorVersion}`, options.root)(options.nested);
