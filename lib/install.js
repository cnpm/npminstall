...
function* _install(parentDir, pkg, ancestors, options) {
  ...
  spinner.text = `Installing ${pkg.name}@${pkg.version}`;
  ...
  try {
    const publishTime = realPkg.publish_time || realPkg._cnpm_publish_time;
    if (publishTime && publishTime >= options.recentlyUpdateMinDateTime) {
      options.recentlyUpdates.set(`${displayName}`, new Date(publishTime));
    }
