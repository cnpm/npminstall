...
function* install(parentDir, pkg, options) {
  ...
  if (info.exists) {
    // make sure bins will be links to ${realPkgDir}/node_modules/.bin
  ...
  if (names.length > 0) {
    ...
    for (const name of names) {
      ...
      tasks.push(_install(realPkgDir, realPkg, childPkg, options));
    }
    yield tasks;
  }
...
function* _install(parentDir, parentPkg, childPkg, options) {
