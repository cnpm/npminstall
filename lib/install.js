...
module.exports = function* install(pkg, options) {
  ...
  if (names.length > 0) {
    ...
    for (const name of names) {
      ...
      const childPkgDir = path.join(nodeModulesDir, name);
      const relativeDir = path.relative(path.dirname(childPkgDir), childPkgRealDir);
      debug('[%s@%s] link %s => %s', pkg.name, pkg.version, childPkgDir, relativeDir);
