...
function* _install(parentDir, pkg, options) {
  ...
  if (c) {
    // make sure package install completed
    // or it may error when using in `postinstall` scripts
    // https://github.com/cnpm/npminstall/issues/31
    if (!c.done) {
  ...
  if (p.type === 'range') {
    ...
    if (max) {
      ...
      if (c) {
        if (!c.done) {
          yield options.events.await(rangeKey);
        }
        const pkg = c.package;
  ...
  options.cache[key] = installCache;

  if (rangeKey) {
    options.cache[rangeKey] = installCache;
  }

  // for peerDependencies: record the latest version, linked at ${storeDir}/node_modules
  ...
  if (info.exists) {
    ...
    installCache.done = true;
    options.events.emit(key);
    rangeKey && options.events.emit(rangeKey);
    return {
      exists: true,
      dir: realPkgDir,
    };
