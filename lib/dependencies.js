...
function checkDumplicate(pkg) {
  ...
  push('buildDependencies');
  push('isomorphicDependencies');

  const duplicates = [];
  for (const [ name, scopes ] of all) {
    if (scopes.length > 1) duplicates.push([ name, scopes ]);
  }

  if (duplicates.length) {
    const detail = duplicates.map(([ name, scopes ]) => `${name} defined multiple times in ${scopes.join(',')}`).join('\n');
