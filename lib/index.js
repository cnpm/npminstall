...
module.exports = function*(options) {
  ...
  options.console.info(chalk.green('All packages installed (%s packages, use %s, avg speed %s/s, total response %s)'),
    options.downloadPackages, ms(totalUse), bytes(avgSpeed), bytes(options.totalResponseSize));
};
...
function* installOne(parentDir, childPkg, options) {
  ...
  if (realDir) {
    const totalUse = Date.now() - options.start;
    const avgSpeed = options.totalResponseSize / totalUse * 1000;
    options.console.info('[%s] installed at %s (%s packages, use %s, avg speed %s/s, total response %s)',
