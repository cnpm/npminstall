...
function finishInstall(options) {
  ...
  const logArguments = [
    chalk[options.detail ? 'green' : 'white']('All packages installed (%s%s%s%sused %s(network %s), speed %s/s, json %s(%s), tarball %s, manifests cache hit %s / miss %s(%s, etag hit %s / miss %s)%s)'),
    ...
    options.totalCacheJSONMissCount,
    bytes(options.totalCacheJSONSize),
