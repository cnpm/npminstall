...
function* _install(options) {
  ...
  const logArguments = [
    chalk[options.detail ? 'green' : 'white']('All packages installed (%s%s%s%sused %s, speed %s/s, json %s(%s), tarball %s)'),
...
function* checkLinkPeerDependencies(params, options) {
  ...
  let rootPkg = {};
  try {
    rootPkg = require(path.join(options.targetDir, 'node_modules', realPkg.name, 'package.json'));
