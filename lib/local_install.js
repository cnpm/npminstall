...
function* shouldOverrideLink(pkg, linkDir, options) {
  const forceLinkLatest = options.forceLinkLatest;
  const rootPkgDependencies = options.rootPkgDependencies;
  ...
  try {
    const pkgJSON = yield utils.readJSON(pkgJSONPath);
...
function* linkLatestVersion(pkg, storeDir, options) {
  ...
  if ((yield fs.exists(linkDir)) && !(yield shouldOverrideLink(pkg, linkDir, options))) {
  ...
  yield utils.rimraf(linkDir); // make sure to delete linkDir
