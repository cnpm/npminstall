...
function* installOne(parentDir, childPkg, options) {
  ...
  if (res) {
    ...
    const avgSpeed = totalSize / totalUse * 1000;
    options.console.info(
      '[%s] %s at %s (%s packages, use %s, speed %s/s, json %s, tarball %s)',
      chalk.green(childPkg.name + '@' + childPkg.version),
  ...
  for (const task of options.postInstallTasks) {
    ...
    try {
      if (installScript) {
        options.console.log(
          '%s %s %j',
          chalk.yellow('scripts.install start'),
        ...
        options.console.log(
          '%s %s successed in %s',
          chalk.yellow('scripts.install finished'),
          chalk.gray(displayName),
          ms(Date.now() - start)
        );
      }
      if (postinstallScript) {
        options.console.log(
          '%s %s %j',
          chalk.yellow('scripts.postinstall start'),
          chalk.gray(displayName),
          postinstallScript
        );
        const start = Date.now();
        ...
        options.console.log(
          '%s %s successed in %s',
          chalk.yellow('scripts.postinstall finished'),
          chalk.gray(displayName),
          ms(Date.now() - start)
        );
      }
