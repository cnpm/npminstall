...
async function localFolder(filepath, pkg, options) {
  ...
  try {
    ...
    debug('install ${pkg.name}@${pkg.rawSpec} with packlist: %s', files.join(','));
    ...
    // fallback to copy
    debug(`install ${pkg.name}@${pkg.rawSpec} from local folder ${filepath} with npm-packlist failed(${err.message})`);
  }
