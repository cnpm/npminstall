...
async function getFullPackageMeta(name, globalOptions) {
  ...
  if (cache.expired > Date.now()) {
    globalOptions.totalCacheJSONSize += cacheContent.length;
    globalOptions.totalCacheJSONCount += 1;
    return cache.manifests;
  }
  // use etag to request
  return await _fetchFullPackageMetaWithCache(pkgUrl, globalOptions,
    cacheFile, cache, cacheContent.length);
}

async function _fetchFullPackageMetaWithCache(pkgUrl, globalOptions, cacheFile, cache, cacheSize) {
  const etag = cache && cache.etag;
  const result = await _fetchFullPackageMeta(pkgUrl, globalOptions, etag);
  ...
  if (result.status === 304) {
    ...
    globalOptions.totalCacheJSONSize += cacheSize;
    globalOptions.totalCacheJSONCount += 1;
    return cache.manifests;
  }
  // 200 status
  ...
  globalOptions.totalCacheJSONMissCount += 1;
  return result.data;
}
