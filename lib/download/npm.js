...
async function getFullPackageMeta(name, options) {
  // check name has scope
  let registry = options.registry;
  const scope = getScope(name);
  if (scope) {
    registry = config.get(scope + ':registry') || options.registry;
  }
...
async function download(pkg, options) {
  ...
  if (!utils.matchPlatform(process.platform, pkg.os)) {
    options.console.warn(`[${pkg.name}@${pkg.version}] skip download for reason ${pkg.os.join(', ')} dont includes your platform ${process.platform}`);
    return {};
  }
