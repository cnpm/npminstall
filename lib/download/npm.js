...
module.exports = function* (pkg, options) {
  ...
  debug('[%s@%s] spec: %s, status %s, real version: %s, headers: %j, responseSize: %s',
    pkg.name, pkg.version, pkg.spec, result.status, realPkg.version, result.headers, result.res.size);
...
function getPackageNpmUri(pkg, options) {
  ...
  const spec = pkg.spec === '*' ? 'latest' : pkg.spec;
  ...
  if (binaryMirror) {
    ...
    if (pkg.scripts && pkg.scripts.install) {
      ...
      if (yield fs.exists(indexFilepath)) {
        ...
        content = content.replace('https://github.com/facebook/flow/releases/download/v', binaryMirror.host);
