...
async function resolve(pkg, options) {
  ...
  if (options.autoFixVersion) {
    ...
    if (fixVersion) {
      ...
      //   }
      // }
      if (fixVersion['allow-scripts'] === false) {
        options.console.warn(`[${pkg.name}@${realPkgVersion}] allow-scripts: ${chalk.green(JSON.stringify(fixVersion['allow-scripts']))}, reason: ${chalk.yellow(fixVersion.reason)}`);
